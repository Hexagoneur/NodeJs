...
io.on('connection', (socket) => {
 ...
 socket.on('create_room', (data) => {
  const { emmet_id, dest_id } = data;

  // Vérification de la présence des deux utilisateurs dans la liste des utilisateurs connectés
  if (!utilisateurs[emmet_id] || !utilisateurs[dest_id]) {
    return;
  }

      // Création d'une room unique pour les deux utilisateurs
      const roomName = `${emmet_id}_${dest_id}`;
      socket.join(roomName);
      console.log(`Room ${roomName} créée`);
  
      // Envoi d'un message aux deux utilisateurs pour les informer de la création de la room
      io.to(utilisateurs[emmet_id]).to(utilisateurs[dest_id]).emit('room_created', { roomName });
    });
